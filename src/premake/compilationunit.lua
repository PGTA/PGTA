...
function premake.extensions.compilationunit.customBakeFiles(base, prj)
	...
	for cfg in project.eachconfig(prj) do
		...
		for i = 1, compilationunit.numcompilationunits do
			table.insert(cfg.files, compilationunit.getCompilationUnitFilename(cfg, i))
		end
	end
...
function premake.extensions.compilationunit.customBakeConfigs(base, sln)
	...
	for config, files in pairs(compilationunit.compilationunits) do
		...
		local units = {}
		for i = 1, compilationunit.numcompilationunits do
			local filename = compilationunit.getCompilationUnitFilename(config, i)
...
--		The full file name
--
function premake.extensions.compilationunit.getCompilationUnitFilename(cfg, index)
